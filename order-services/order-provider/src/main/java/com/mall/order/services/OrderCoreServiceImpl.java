  ...
  /*      pipeline.addLast(validateHandler);
    ...
    public PayOrderSuccessResponse payOrderSuccess(PayOrderSuccessRequest request) {
        ...
        try {
            ...
            orderItemMapper.updateStockStatus(OrderConstants.ORDERITEM_STATUS_STOCK_DEDUCTION, orderId, new Date());
            List<OrderItem> orderItems = orderItemMapper.queryByOrderId(orderId);
            for (OrderItem orderItem : orderItems) {
                Long itemId = orderItem.getItemId();
                Integer num = orderItem.getNum();
                ...
                stock.setLockCount(-num);
                /**
                 * 下面这个bug挺难发现的，即便stock不更新，但是我们也要将它赋0
                 * 如果我们不赋值，那么Long初始为null， 那么sql语句就会出问题
                 */
            ...
            log.error("OrderCoreServiceImpl.payOrderSuccess occurs Exception :" + e);
            ExceptionProcessorUtils.wrapperHandlerException(response, e);
        }
        return response;
    }
    ...
    @Override
    public PayOrderFailResponse payOrderFail(PayOrderFailRequest request) {
//        PayOrderFailResponse response = new PayOrderFailResponse();
//        try {
//            request.requestCheck();
//            String orderId = request.getOrderId();
//            Order order = new Order();
//            order.setOrderId(orderId);
//            order.setStatus(OrderConstants.orderstatu);
//        } catch (Exception e) {
//			log.error("OrderCoreServiceImpl.payOrderFail occurs Exception :" + e);
//            ExceptionProcessorUtils.wrapperHandlerException(response, e);
//        }
//        return response;
//    }
